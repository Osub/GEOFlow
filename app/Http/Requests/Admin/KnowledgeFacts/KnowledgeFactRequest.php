    ...
    public function rules(): array
    {
        return match ((string) $this->route()?->getName()) {
            'admin.knowledge-bases.facts.store' => [
                ...
                'predicate' => ['required', 'string', 'max:255'], 'value_type' => ['required', 'in:string,integer,decimal,number,date,boolean,url'],
                'locale' => ['sometimes', 'string', 'max:16'], 'aliases_json' => ['sometimes', 'array'],
            ...
            'admin.knowledge-bases.facts.update' => [
                ...
                'aliases_json' => ['sometimes', 'array'], 'importance' => ['sometimes', 'in:critical,high,normal'],
            ...
            'admin.knowledge-bases.fact-values.store' => [
                ...
                'comparison_policy_json' => ['sometimes', 'array'], 'review_status' => ['sometimes', 'in:draft,reviewed,rejected'],
            ],
            'admin.knowledge-bases.fact-values.update' => [
                'lock_version' => ['required', 'integer', 'min:1'], 'canonical_value_json' => ['sometimes', 'array'], 'canonical_answer' => ['sometimes', 'string', 'max:5000'],
                'temporal_kind' => ['sometimes', 'in:timeless,observed,interval'], 'scope_json' => ['sometimes', 'array'], 'valid_from' => ['nullable', 'date'],
                'valid_to' => ['nullable', 'date'], 'observed_at' => ['nullable', 'date'], 'comparison_policy_json' => ['sometimes', 'array'],
